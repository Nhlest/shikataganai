...
impl<T> Array2d<T> {
  pub fn new_zeroed(((x0, y0), (x1, y1)): ((usize, usize), (usize, usize))) -> Array2d<T> {
    let size = (x1 - x0 + 1) * (y1 - y0 + 1);
  ...
  pub fn new_init<F: Fn((usize, usize)) -> T>(
  ...
  pub fn in_bounds(&self, (x, y): (usize, usize)) -> bool {
    let ((x0, y0), (x1, y1)) = self.bounds;
    x >= x0 && x <= x1 && y >= y0 && y <= y1
  }
  pub fn map_in_place<F: Fn((usize, usize), &T) -> T>(&mut self, f: F) {
  ...
  pub fn map<O, F: Fn((usize, usize), &T) -> O>(&self, f: F) -> Array2d<O> {
    Array2d::new_init(self.bounds, |x| f(x, &self[x]))
  }
  pub fn foreach<F: FnMut((usize, usize), &T)>(&self, mut f: F) {
    let ((x1, y1), (x2, y2)) = self.bounds;
    for ix in x1..x2 {
      for iy in y1..y2 {
        f((ix, iy), &self[(ix, iy)]);
...
impl<T> Index<(usize, usize)> for Array2d<T> {
  ...
  fn index(&self, (x, y): (usize, usize)) -> &Self::Output {
    ...
    let row = x1 - x0 + 1;
    unsafe { &*self.data.add((y - y0) * row + (x - x0)) }
  }
}
...
impl<T> IndexMut<(usize, usize)> for Array2d<T> {
  fn index_mut(&mut self, (x, y): (usize, usize)) -> &mut Self::Output {
    ...
    assert!(x0 <= x && x <= x1 && y0 <= y && y <= y1, "Array out of bounds");
    let row = x1 - x0 + 1;
    unsafe { &mut *self.data.add((y - y0) * row + (x - x0)) }
  }
}
